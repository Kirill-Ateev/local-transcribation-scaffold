...
- Авторизация: по умолчанию все запросы транскрибации требуют
  `Authorization: Bearer <TRANSCRIBE_TOKEN>`. Если клиент не умеет слать
  заголовок (OpenWhispr Self-Hosted) — задайте `AUTH_REQUIRED=0` в
  `server/.env` и перезапустите контейнер; допустимо только при доступности
  сервиса исключительно из доверенной LAN. Текущий режим виден в `/health`
